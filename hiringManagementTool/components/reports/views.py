...
class OpenPositionReportView(APIView):
    """
    API endpoint that returns aggregated open positions data as JSON.
    """
    def get(self, request, format=None):
        data = get_open_positions_by_department_lob()  # Execute the raw SQL query
        serializer = OpenPositionSerializer(data=data, many=True)
        serializer.is_valid(raise_exception=True)
        return Response(serializer.data)
